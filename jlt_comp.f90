...
module jlt_comp
  use jlt_constant, only : STR_SHORT, STR_MID, MAX_MODEL, MAX_DOMAIN, CONF_FILE
  implicit none
  private 
...
subroutine set_global_component(total_comp_name)
  use jlt_mpi_lib, only : jml_BcastGlobal
  implicit none
...
subroutine search_next_process(current_process, comp_name, num_of_comp, next_process)
  use jlt_mpi_lib, only : jml_BcastGlobal, jml_AllReduceMin
  implicit none
...
subroutine add_comp_name(current_process, comp_name, num_of_comp)
  use jlt_mpi_lib, only : jml_SendGlobal, jml_RecvGlobal
  implicit none
  integer, intent(IN) :: current_process
  character(len=STR_SHORT), intent(INOUT) :: comp_name(MAX_MODEL)
  integer, intent(INOUT) :: num_of_comp
...
subroutine init_my_component_info(total_comp_name)
  use jlt_mpi_lib, only : jml_AllreduceMax
...
subroutine set_component_info()
  use jlt_mpi_lib, only : jml_GetLeaderRank, jml_GetCommSizeLocal  
  implicit none
...
subroutine init_model_process()
  use jlt_mpi_lib, only : jml_init, jml_GetCommSizeGlobal, jml_GetMyrankGlobal, jml_create_communicator
  use jlt_utils, only : error
  !use jlt_time, only : init_all_time
  implicit none
...
integer function get_component_relation(my_component_id, target_component_id)
  use jlt_constant, only : COMP_PARALLEL, COMP_SERIAL, COMP_SUPERSET, COMP_SUBSET, COMP_OVERLAP
  implicit none
